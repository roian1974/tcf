...
def getThresholds():
    ...
    batch_placeholder = tf.placeholder(tf.float32, shape=(4, params.batch_size))
    corrupt_placeholder = tf.placeholder(tf.bool, shape=(1)) #Which of e1 or e2 to corrupt?
    predictions_list = ntn.inference(batch_placeholder, corrupt_placeholder, init_word_embeds, entity_to_wordvec, num_entities, num_relations, slice_size, params.batch_size)
    ...
    while(score <= max_score):
        ...
        for i in range(params.num_relations):
            ...
            accuracy = tf.reduce_mean((predictions == dev_data[relations_list, 0]))
