...
def COM1002():
    rtn = True
    try:
        comlogging.logger.info( "COM1002_start() start ")
        if COM1002_start() == True :
            comlogging.logger.info( "COM1002_processing() start ")
            COM1002_processing()
        else :
            raise Exception('COM1002_start 함수에서 에러가 발생')
    except Exception as err:
        comlogging.logger.error( 'COM1002-'+ str(err))
        include.setErr('ECOM1002', 'COM1002' + str(err))
    else:
        comlogging.logger.info( 'COM1002-성공')
    finally:
        COM1002_end()
...
def COM1002_start() :
    try:
        ...
        if len(include.getArgv()) != 4:
            comutil.ferrprint(__file__, 'COM1002_start', '인자가 부족합니다.' + str(len(sys.argv)))
            include.setErr('ECOM031', 'COM1002_start,인자가 부족합니다.' + str(len(sys.argv)))
            raise Exception('COM1002_start,인자가 부족합니다.')
        ...
        comlogging.logger.error( 'COM1002_start-' + str(err))
        include.setErr('ECOM1002', 'COM1002_start,' + str(err))
    else:
        comlogging.logger.info( 'COM1002_start-성공')
    finally:
        if include.isError() == False :
            return True
        else :
...
def COM1002_end() :
    try:
        ...
        comlogging.logger.error( 'COM1002_end '+ str(err))
        include.setErr('ECOM1002', 'COM1002_end,' + str(err))
    else:
        comlogging.logger.info( 'COM1002_end-성공')
    finally:
