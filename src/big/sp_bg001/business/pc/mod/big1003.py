...
def BIG1003_processing(cdto):
    try:
        comlogging.logger.info("#################################################################" + str(getIntTime()))

        if cdto.indata['domain_function'] == 'handdataDownload' :
            dc_bg001.handdataDownload(cdto)
        elif cdto.indata['domain_function'] == 'toCSV' :
            dc_bg001.toCSV(cdto)
        elif cdto.indata['domain_function'] == 'handTypeModel':
            cdto = dc_bg001.preAnalysis_handTypeModel(cdto)
            if cdto != True:
                cdto = dc_bg001.trainModel_handTypeModel(cdto)
        elif cdto.indata['domain_function'] == 'catchLANG':
            cdto = dc_bg001.preAnalysis_catchLANG(cdto)
            if cdto != True:
                cdto = dc_bg001.trainModel_catchLANG(cdto)
