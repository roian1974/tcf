# ----------------------------------------------------------------------------------------------------------------------
# 작성일   : 2018년01월20일
# 모듈내용 : 트랜잭션 메인 관리 엔진으로 Front Controller에 해당되는 부분이다.
#             이 모듈은 업무 컴포넌트당 하나씩 만들어서 배포관리 한다.
# 작성자   : sk c&c 2018-02-02 
# ----------------------------------------------------------------------------------------------------------------------
...
# python tcf_sp_if001 sp_if001 ITF1000 3 : 인터페이스 테이블을 읽어서 작업의 단위를 3건단위로 해서 처리한다. 멀티프로세스 타입
# python tcf_sp_if001 sp_if001 ITF1002 3 : 멀티쓰레드 타입

def TCF_SP_if001():

    include.showCominfo()

    comlogging.logger.info("TCF_SP_if001 모듈시작")
    ...
    try:
        ...
        comlogging.logger.error( 'TCF_SP_if001 에서 발생 :' + str(err))
        include.setErr('ECOM001', 'TCF Error-' + str(err))
    else:
        comlogging.logger.info( "SUS:F[" + 'TCF_SP_if001' + ']함수 호출성공')
    finally:
...
def tpsvrinit(argv):
    ...
    comlogging.logger.info( "============================================= TCF_SP_if001() s")
...
def commandmain() :
    ...
    if rtn == False:
        ...
        TCF_SP_if001()
    tpsvrdone()
    comlogging.logger.info( "============================================= TCF_SP_if001() e")
    include.showCominfo()
