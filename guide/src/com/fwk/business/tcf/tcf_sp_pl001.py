...
def TCF_SP_pl001():
    ...
    comlogging.logger.info("TCF_SP_pl001 모듈시작")

    try:
        comlogging.logger.debug( "=================== STF() 함수호출 ")
        comlogging.logger.debug( "STF() start ")
        stf_common.STF()
        comlogging.logger.debug( "STF() end ")

        if include.isError() == False :
            comlogging.logger.debug( "STF() 함수호출성공 ")
            comlogging.logger.debug( "===================AP [%s] main start " % (include.getServiceName()))

            spaction.action(include.getServiceName())

            comlogging.logger.debug( "===================AP [%s] main end " % (include.getServiceName()))

            if include.isError() == False :
                comlogging.logger.debug( "AP[%s] 함수호출성공" % (include.getServiceName()))
            else:
        ...
        comlogging.logger.error( 'TCF_SP_pl001 에서 발생 :' + str(err))
        include.setErr('ECOM001', 'TCF Error-' + str(err))
    else:
        comlogging.logger.debug( "SUS:F[" + 'TCF_SP_pl001' + ']함수 호출성공')
    finally:
        comlogging.logger.debug( "=================== ETF() 함수호출 ")
        comlogging.logger.debug( "ETF() start ")
        etf_common.ETF()
        comlogging.logger.debug( "ETF() end ")
...
def tpsvrinit(argv):
    ...
    comlogging.logger.debug( "============================================= TCF_SP_pl001() s")
    comlogging.logger.debug( 'DB connect 호출(dbconnect)')
    ...
    if rtn == -1:
        ...
        comlogging.logger.debug( "dbconnect() call success")
    ...
    if rtn == -1:
        ...
        comlogging.logger.debug( "tpsinit() call success")
...
def commandmain() :
    ...
    if rtn == False:
        ...
        TCF_SP_pl001()
    tpsvrdone()
    comlogging.logger.debug( "============================================= TCF_SP_pl001() e")
...
def main(argv):
    ...
    if rtn == False:
        ...
        TCF_SP_pl001()
    tpsvrdone()
    comlogging.logger.info( "============================================= TCF_SP_pl001() e")
