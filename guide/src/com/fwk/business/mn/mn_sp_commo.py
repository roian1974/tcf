...
def MN_SP_commo():
    ...
    comlogging.logger.info("MN_SP_commo 모듈시작")
    ...
    try:
        ...
        comlogging.logger.error( 'MN_SP_commo 에서 발생 :' + str(err))
        include.setErr('ECOM001', 'TCF Error-' + str(err))
    else:
        comlogging.logger.info( "SUS:F[" + 'MN_SP_commo' + ']함수 호출성공')
    finally:
...
def tpsvrinit(argv):
    ...
    comlogging.logger.info( "============================================= MN_SP_commo() s")
...
def commandmain() :
    ...
    if rtn == False:
        ...
        MN_SP_commo()
    tpsvrdone()
    comlogging.logger.info( "============================================= MN_SP_commo() e")
